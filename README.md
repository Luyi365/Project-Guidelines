...
本仓库作为规范总入口，各语言的具体规范与 Skill 以**独立仓库**形式维护，并通过 Git submodule 挂载到 `skills/` 目录下。
...
由于 Skill 以 submodule 形式引入，克隆时需要一并拉取子模块：
...
`.gitmodules` 中的子模块使用**相对地址**（`../c-code-review.git`），会按父仓库的 remote 自动解析：从 GitHub 克隆则走 GitHub，从 Gitee 克隆则走 Gitee，两条链路都无需切换配置。

若只需要 C 代码规范，也可以单独克隆 Skill 仓库，无需本仓库：
...
> 注意：规范原文已迁移至各自的独立仓库，请勿再使用 `Project-Guidelines/skills/...` 路径下的 raw 链接（submodule 内容不随父仓库分发，该类链接均已失效）。Gitee 镜像为单向同步，内容可能略滞后于 GitHub。
